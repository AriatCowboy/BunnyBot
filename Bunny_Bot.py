...
prefix = '!'
TOKEN = "NjgwMzMwODE4NDEzMTMzODU0.XlDgOw.tVfaCOcAmx1CTD81ZGRoqOgsCiw"
...
def Get_update_cat(message):
	guild_ID = message.guild.id
	for cat in message.guild.categories:
		cat_ID = cat.id
		cat_name = cat.name
		cat_position = cat.position
		update_category_info(cat_ID, cat_name, cat_position, guild_ID)

def Get_update_text_channels(message):
	guild_ID = message.guild.id
...
def Get_update_voice_channels(message):
	guild_ID = message.guild.id
...
async def on_member_join(member):
	...
	for channel in member.guild.channels:
		if str(channel) == "chat":
			await channel.send("{} has joined the community. Nice for you to hop in!".format(member.name))
...
async def addsl(ctx):
	...
	if author.id == message.guild.owner.id or str(author) in valid_users or author.guild_permissions.administrator:
		...
		try:
			...
			if reply:
				await ctx.send("{} {} - {} was added to the song database!".format(author, Artist[:num], Song[1:]))
			else:
				await ctx.send("{} Please use the format Artist - Song.".format(author))
...
async def on_message(message):
	if message.author == client.user:
		return
	bot_channel_check = readbotchannel(message)
	bot_channel = bot_channel_check
	await update_database(message)
	update_message_count(message)
	author = message.author
	content = message.content
	channel = message.channel
	...
	if content.startswith("!mod_user") or content == "!logoff" or content.startswith("!bot_channel"):
		if author.id == authorized_users[0]:
		...
		if content.startswith("!ss") or content.startswith("!suggestions") or content.startswith("!clear"):
			if content.startswith("!suggestions") or content.startswith("!clear") and author.id == authorized_users:
				await client.process_commands(message)
			elif content.startswith("!ss"):
				await client.process_commands(message)
	elif channel.id == bot_channel.id:
		print(bot_channel)
		await client.process_commands(message)
	elif message.startswith("!"):
		await message.author.send("you entered an invalid command")
...
class YTDLSource(discord.PCMVolumeTransformer):
	def __init__(self, source, *, data, volume=.05):
	...
	async def from_url(cls, url, *, loop=None, stream=False):
		...
		print(data["extractor"] + "-" + data["id"])
		print(ytdl.prepare_filename(data))
